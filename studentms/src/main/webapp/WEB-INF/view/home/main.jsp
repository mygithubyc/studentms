...
<link rel="stylesheet" href="${ctx}/static/css/home/main.css">
<link rel="stylesheet"
	href="${ctx}/static/js-modules/easyui/themes/IconExtension.css">
</head>
<body>
	...
	<div class="header">
		<div class="header__wrap">
			<c:if test="${user.userType==2 }">
				<div class="header__logo">学生管理平台 欢迎你,${user.username}老师</div>
			</c:if>
			<c:if test="${user.userType==3 }">
				<div class="header__logo">学生管理平台 欢迎你,${user.username}同学</div>
			</c:if>
	...
	<div id="main_content"
		...
		<c:choose>
			<c:when test="${user.userType==2 }">
				<span id="teacher_job" class="add_tab_span"
					url="${ctx }/teacher/job" title="布置作业">布置作业</span>
				<span id="teacher_marks" class="add_tab_span"
					url="${ctx }/teacher/mark" title="录入成绩">录入成绩</span>
				<span id="teacher_plan" class="add_tab_span"
					url="${ctx }/teacher/plan" title="下载教学计划">下载教学计划</span>
		...
		<c:choose>
			<c:when test="${user.userType==3 }">
				<span id="student_job" class="add_tab_span"
					url="${ctx }/student/job" title="提交作业">提交作业</span>
				<span id="student_marks" class="add_tab_span"
					url="${ctx }/student/mark" title="成绩查询">成绩查询</span>
			</c:when>
			<c:otherwise></c:otherwise>
		</c:choose>
		<!-- 学生菜单栏结束 -->
		...
		<span id="exam_menu">考试安排</span> <span id="job_menu">作业情况</span> <span
			id="message_menu">班主任通知</span>
		...
		<c:when test="${user.userType==3}">
			<div id="message_dlg" class="dlg">
				...
				<form id="message_fm" class="fm" method="post">
					<div class="fitem">
						<label>内容:</label> <input name="">
					</div>
					<div class="fitem">
						<label>通知等级:</label> <select name="" id="" style="width: 140px;">
					...
					<div class="fitem">
						<label>过期时间:</label> <input name="" class="easyui-datebox"
							editable="false">
					</div>
					<div class="fitem">
						<label>附件:</label> <input name="">
					</div>
			...
			<table cellPadding="10px" cellSpacing="5px">
				...
				<tr>
					...
					<td colspan="2" rowspan="2"><img
						src="${ctx }/static/img/zhaopian.jpg" alt="" id="user_image">
					</td>
				</tr>
				<tr>
...
<script src="${ctx}/static/js/home/main.js"></script>
</html>